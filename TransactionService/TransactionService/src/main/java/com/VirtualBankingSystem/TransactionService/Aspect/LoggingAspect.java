package com.VirtualBankingSystem.TransactionService.Aspect;
...
public class LoggingAspect {
    ...
    public void logRequest(JoinPoint joinPoint) {
        try {
            if (joinPoint.getArgs().length > 0) {
                ...
                logNode.put("message", objectMapper.writeValueAsString(requestBody));
    ...
    public void logResponse(JoinPoint joinPoint, Object response) {
        try {
            ObjectNode logNode = objectMapper.createObjectNode();
            logNode.put("message", objectMapper.writeValueAsString(response));
